...
def _init_wandb_run(args, train=True):
    ...
    if train and args.ddp and args.wandb.resume:
        ...
        wandb.init(
            ...
            id=f"{group_id}-worker-{args.rank}",
            job_type="worker",
            resume="allow",
...
def log_data_to_wandb(
    ...
    enhanced_spectrogram_wandb_image = wandb.Image(convert_spectrogram_to_heatmap(enhanced_spectrogram), caption="PR")
    ...
    if epoch <= 10:
        ...
        hr_enhanced_spectrogram_wandb_image = wandb.Image(
            convert_spectrogram_to_heatmap(hr_enhanced_spectrogram), caption="HR"
        )
        hr_enhanced_wandb_audio = wandb.Audio(hr_signal.squeeze().numpy(), sample_rate=hr_sr, caption="HR")
        wandb_dict.update(
            {
                f"test samples/{filename}/{hr_name}_spectrogram": hr_enhanced_spectrogram_wandb_image,
        ...
        lr_enhanced_spectrogram_wandb_image = wandb.Image(
            convert_spectrogram_to_heatmap(lr_enhanced_spectrogram), caption="LR"
        )
        lr_enhanced_wandb_audio = wandb.Audio(lr_signal.squeeze().numpy(), sample_rate=lr_sr, caption="LR")
        wandb_dict.update(
            {
                f"test samples/{filename}/{lr_name}_spectrogram": lr_enhanced_spectrogram_wandb_image,
