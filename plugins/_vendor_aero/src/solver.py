...
class Solver:
    def __init__(self, data, models, optimizers, args):
        ...
        if "discriminator_model" in self.args.experiment and self.args.experiment.discriminator_model == "hifi":
            self.melspec_transform = torchaudio.transforms.MelSpectrogram(
                self.args.experiment.hr_sr, **self.args.experiment.mel_spectrogram
            ).to(self.device)
    ...
    def train(self):
        ...
        for epoch in range(len(self.history), self.epochs):
            ...
            logger_msg = f"Train Summary | End of Epoch {epoch + 1} | Time {time.time() - start:.2f}s | " + " | ".join(
                [f"{k} Loss {v:.5f}" for k, v in losses.items()]
            )
            ...
            if (
                self.cross_valid
                and ((epoch + 1) % self.cross_valid_every == 0 or epoch == self.epochs - 1)
                and self.cv_loader
            ):
                # Cross validation
                ...
                with torch.no_grad():
                    ...
                    if self.args.valid_equals_test:
                        enhance_valid_data = (epoch + 1) % self.eval_every == 0 or (
                            epoch == self.epochs - 1 and self.tt_loader
                        )
                        valid_losses, enhanced_filenames = self._get_valid_losses_on_test_data(
                            epoch, enhance=enhance_valid_data
                        )
                ...
                logger_msg = (
                    f"Validation Summary | End of Epoch {epoch + 1} | Time {time.time() - cross_valid_start:.2f}s | "
            ...
            if ((epoch + 1) % self.eval_every == 0 or epoch == self.epochs - 1) and self.tt_loader:
                ...
                with swap_state(self.model, best_state):
                    ...
                    if evaluated_on_test_data:
                        ...
                        enhanced_dataloader = distrib.loader(
                            enhanced_dataset, batch_size=1, shuffle=False, num_workers=self.args.num_workers
                        )
        ...
        for i, data in enumerate(logprog):
            ...
            if enhance:
                save_wavs(
                    ...
                    lr,
                    hr,
                    [os.path.join(self.args.samples_dir, filename)],
                    self.args.experiment.lr_sr,
