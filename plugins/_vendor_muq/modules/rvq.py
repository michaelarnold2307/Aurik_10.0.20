        ...
        if n_layer == 1:
            ...
            self.in_proj = nn.Sequential(
                ...
                *[
                    nn.Sequential(
                        nn.ReLU(),
                        ProjClass(ndim_hidden, ndim_hidden, kernel_size=1),
                    )
            ...
            self.out_proj = nn.Sequential(
                ...
                nn.ReLU(),
                *[
                    nn.Sequential(ProjClass(ndim_hidden, ndim_hidden, kernel_size=1), nn.ReLU())
                    for _ in range(n_layer - 2)
                ],
    ...
    def decode_latents(self, latents):
        ...
        if self.training:
            ...
            if replace_code.sum(-1) > 0:
                print(f"Replace {replace_code.sum(-1)} codes")
                ...
                self.codebook.weight.data = self.codebook.weight.data * (1 - replace_code).unsqueeze(
                    -1
                ) + random_input * replace_code.unsqueeze(-1)
