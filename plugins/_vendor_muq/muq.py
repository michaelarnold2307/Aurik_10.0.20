...
class MuQConfig:
    ...
    stat: dict | None = field(default_factory=dict)
    w2v2_config: dict | None = field(default_factory=dict)
    use_rvq_target: bool = field(default=False)
    use_vq_target: bool = field(default=False)
    use_encodec_target: bool = field(default=False)
    rvq_ckpt_path: str | None = field(default=None)
    recon_loss_ratio: float | None = field(default=None)
...
class MuQ(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(
        self, x, attention_mask: torch.Tensor | None = None, output_hidden_states: bool = True
    ) -> BaseModelOutput:
        """
        ...
        return BaseModelOutput(last_hidden_state=last_hidden_state, hidden_states=hidden_states)
