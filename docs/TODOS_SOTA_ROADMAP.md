...
| ANA-8 | **IPA-Formant-Ziele tot**: `has_ipa` immer False (Decoder-ONNX fehlt) → `formant_target_for_range()` liefert immer None → phase_56-Formant-Ziel für vokal-dominante Segmente ist INERT (Doku-Head des Moduls bestätigt) | BACKLOG (Modell/extern): IPA-Decoder-ONNX beschaffen oder Vokal-Formanten direkt aus LPC schätzen (deterministischer CPU-Ersatz möglich — F1/F2-LPC-Roots sind bereits in `_detect_language` implementiert) |
| ANA-9 | **Niedrig-konfidente Phonem-Segmente treiben Gates ungefiltert**: Timeline-Segmente aus einer 3-%-Transkription steuern trotzdem Segment-Gates in 19/24/43/56 (confidence wird nur gespeichert, nie gegated) | BACKLOG (Rezept): conf < Schwelle ⇒ Klasse auf „silence“ setzen (deterministisch, non-blocking); Schwelle empirisch auf dem Elke-Lauf kalibrieren |
| ANA-10 | **Zweite Phonem-Quelle dupliziert** (§V7 (copilot-instructions.md)): `lyrics_guided_enhancement.get_phoneme_mask` (DSP-Phonemkarte, hop 512) vs. PhonemeTimeline (Transkription) — zwei Phonem-Repräsentationen; zusätzlich `styles_zones` mit conf=0,00 im Lauf (Style-Führung inert) | BACKLOG: Rollen klären/kanonisieren; LGE-Saliency- vs. Timeline-Konsum dokumentieren |
