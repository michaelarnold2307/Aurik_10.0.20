...
class FinalEQ(PhaseInterface):
    ...
    def process(
        ...
        **kwargs,
        ...
        _material_key = (
            material if isinstance(material, MaterialType) else MaterialType(material)
        )  # §v10.996 Enum-Normalisierung
        config = {
            k: dict(v) for k, v in self.EQ_CONFIG.get(_material_key, self.EQ_CONFIG[MaterialType.CD_DIGITAL]).items()
        }
