...
class LightweightDecoder(nn.Module):
    ...
    def __init__(self, whisper_dim=384, cond_dim=128):
        super().__init__()
...
class ConditionedUNet(nn.Module):
    ...
    def __init__(self, cond_dim=128):
        ...
        self.bottleneck = nn.Sequential(
            nn.Conv2d(128 + c, 128, 3, padding=1),
            nn.GELU(),
            nn.Conv2d(128, 128, 3, padding=1),
            nn.GELU(),
        )

        self.dec3_conv = nn.Sequential(nn.Conv2d(128, 64, 3, padding=1), nn.GELU())
        self.dec2_conv = nn.Sequential(nn.Conv2d(64 + 64, 32, 3, padding=1), nn.GELU())
        self.dec1_conv = nn.Sequential(nn.Conv2d(32 + 32, 2, 3, padding=1))
    ...
    def forward(self, spec, condition):
        ...
        cond = F.interpolate(cond, size=(e3.shape[2], e3.shape[3]), mode="bilinear", align_corners=False)

        b = self.bottleneck(torch.cat([e3, cond], dim=1))
...
class WhisperDenoiser(nn.Module):
    def __init__(self, device="cpu"):
        ...
        self.decoder = LightweightDecoder(whisper_dim=384, cond_dim=128)
        self.unet = ConditionedUNet(cond_dim=128)
    ...
    def compute_loss(self, noisy_audio, clean_audio):
        ...
        # Spectral MSE on complex STFT
        loss_mse = F.mse_loss(pred_spec.real, clean_spec_gt.real) + F.mse_loss(pred_spec.imag, clean_spec_gt.imag)
